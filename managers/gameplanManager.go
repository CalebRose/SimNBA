...
func CheckAllUserGameplans() {
	...
	for _, team := range teams {
		...
		if totalMinutes <= 199 || totalShotAllocation < 99 || totalShotAllocation > 100 || totalMinutes > 200 {
			message := fmt.Sprintf("%s did not properly gameplan: %d of 200 Minutes and %d of 100 shot allocation were allocated across your roster\n", team.Team, totalMinutes, int(totalShotAllocation))
			// fmt.Printf("%s did not properly gameplan: %d of 200 Minutes and %d of 100 shot allocation were allocated across your roster\n", team.Team, totalMinutes, int(totalShotAllocation))
	...
	for _, team := range nbaTeams {
		if team.NBAOwnerName == "" || team.LeagueID != 1 {
			continue
		}
		totalMinutes := 0
		insideShotAllocation := 0.0
		...
		pfMinutes := 0
		cMinutes := 0
		roster := nbaPlayerMapByTeamID[team.ID]

		for _, player := range roster {
			if player.IsGLeague || player.IsInjured {
				continue
			}
			positionOne := player.PositionOne
			positionTwo := player.PositionTwo
		...
		if totalMinutes <= 239 || totalShotAllocation < 99 || totalShotAllocation > 100 || totalMinutes > 240 {
			message := fmt.Sprintf("%s did not properly gameplan: %d of 240 Minutes and %d of 100 shot allocation were allocated across your roster\n", team.Team, totalMinutes, int(totalShotAllocation))
			// fmt.Printf("%s did not properly gameplan: %d of 200 Minutes and %d of 100 shot allocation were allocated across your roster\n", team.Team, totalMinutes, int(totalShotAllocation))
			CreateNotification("NBA", message, "Gameplan", team.ID)
			nbaTeamIDsToCheck = append(nbaTeamIDsToCheck, team.ID)
			nbaTeamIDStrings = append(nbaTeamIDStrings, strconv.Itoa(int(team.ID)))
		}
	}

	if len(nbaTeamIDStrings) > 0 {
		db.Model(&structs.NBAPlayer{}).Where("team_id in (?)", nbaTeamIDStrings).Updates(map[string]interface{}{
			"minutes":                0,
			"p1_minutes":             0,
			"p2_minutes":             0,
			"p3_minutes":             0,
	...
	for _, teamID := range nbaTeamIDsToCheck {
		team := nbaTeamMap[teamID]
		team.MarkTeamPenalty()
		repository.SaveNBATeamRecord(team, db)
		SetNBAMinutesAndShotProportions(db, teamID, nbaGameplanMap, nbaPlayerMapByTeamID)
	}
}
...
func SetAIGameplans() bool {
	...
	for _, team := range islTeams {
		...
		SetNBAMinutesAndShotProportions(db, team.ID, nbaGameplanMap, nbaPlayerMapByTeamID)
	}
...
func SetCollegeMinutesAndShotProportions(db *gorm.DB, teamID uint, gameplanMap map[uint]structs.Gameplan, collegePlayerMapByTeamID map[uint][]structs.CollegePlayer) {
	...
	for i := 0; i < len(roster); i++ {
		roster[i].UpdatePlayer(0, 0, 0, "", "", "", 0, 0, 0)
...
func SetNBAMinutesAndShotProportions(db *gorm.DB, teamID uint, gameplanMap map[uint]structs.NBAGameplan, nbaPlayerMapByTeamID map[uint][]structs.NBAPlayer) {
	...
	teamTotalSkill := 0
	for i := 0; i < len(roster); i++ {
		if roster[i].Minutes == 0 || roster[i].IsGLeague || roster[i].IsInjured {
			continue
		}
		teamTotalSkill += roster[i].Shooting2 + roster[i].Shooting3 + roster[i].Finishing
	}
	...
	for _, r := range roster {
		repository.SaveNBAPlayerRecord(r, db)
	}
